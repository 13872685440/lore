...
public abstract class BaseProcess {
	...
	public String submit_process(ProcessBean p_parms_bean, ProcessStep next_step, List<String> userIds,
		...
		if ("0".equals(p_parms_bean.getBiaozhi())) {
			// 保存任务
			saveTask(userIds,
					NameQueryUtil.setParams_s("step_id", next_step.getId(), "key_value", p_parms_bean.getEntity_id(),
							"sqr_id", p_parms_bean.getSqr_id(), "previous_id", p_parms_bean.getSqr_id()));
			// 保存意见
			saveComment(NameQueryUtil.setParams_s("step_id", next_step.getScId() + "0000", "key_value",
					p_parms_bean.getEntity_id(), "shyj",
					StringUtil.isEmpty(p_parms_bean.getShyj()) ? "" : p_parms_bean.getShyj(), "shjl", "提交", "sqr_id",
					p_parms_bean.getSqr_id(), "organ_id", p_parms_bean.getOrgan_id()));
			// 更新实体
			updataEntity(next_step.getSc_TableName(),
					StringUtil.isMapEmpty(frist)
							? NameQueryUtil.setParams_s(
									new String[] { "step_id", "step", "step_name", "conclusion", "id" },
									new Object[] { next_step.getScId(), next_step.getStep(), next_step.getName(),
											"提交申请", p_parms_bean.getEntity_id() })
							: frist);
		} else {
			// 重新提交
			...
			if (StringUtil.isEmpty(p_parms_bean.getTask_id())) {
				...
				current_task = (TaskExt) baseService.getSimple("TaskExt", null, true,
						NameQueryUtil.setParams("key_value", p_parms_bean.getEntity_id(), "step_id",
								p_parms_bean.getStep_id() + "0000", "taskType", TaskType.待办));
			} else {
				// 否则 根据task_id 来查找
			...
			saveTask(userIds,
					NameQueryUtil.setParams_s("step_id", next_step.getId(), "key_value", p_parms_bean.getEntity_id(),
							"sqr_id", current_task.getSqr_id(), "previous_id", p_parms_bean.getSqr_id()));
			current_task.setTaskType(TaskType.已办);
			baseService.save(current_task);
			saveComment(NameQueryUtil.setParams_s("step_id", next_step.getScId() + "0000", "key_value",
					p_parms_bean.getEntity_id(), "shyj",
					StringUtil.isEmpty(p_parms_bean.getShyj()) ? "" : p_parms_bean.getShyj(), "shjl", "重新提交", "sqr_id",
					p_parms_bean.getSqr_id(), "organ_id", p_parms_bean.getOrgan_id()));
			updataEntity(next_step.getSc_TableName(),
					StringUtil.isMapEmpty(last)
							? NameQueryUtil.setParams_s(
									new String[] { "step_id", "step", "step_name", "conclusion", "id" },
									new Object[] { next_step.getScId(), next_step.getStep(), next_step.getName(),
											"重新提交申请", p_parms_bean.getEntity_id() })
							: last);
		}
		pushController.pushMessage(p_parms_bean.getSqr_id(), 1);
	...
	public String to_0(TaskExt current_task, ProcessBean p_parms_bean, ProcessStep current_step,
			StepCondition stepCondition, List<String> userIds, Map<String, Object> maps) {

		// 保存任务
		saveTask(Arrays.asList(current_task.getSqr_id()),
				NameQueryUtil.setParams_s("step_id", current_step.getScId() + "0000", "key_value",
						current_task.getKey_value(), "sqr_id", current_task.getSqr_id(), "previous_id",
						p_parms_bean.getSqr_id()));

		// 保存意见
		saveComment(NameQueryUtil.setParams_s("step_id", current_step.getId(), "key_value", current_task.getKey_value(),
				"shyj", p_parms_bean.getShyj(), "shjl", stepCondition.getConclusion(), "sqr_id",
				p_parms_bean.getSqr_id(), "organ_id", p_parms_bean.getOrgan_id()));
		// 当前任务处理为已办
		current_task.setTaskType(TaskType.已办);
		baseService.save(current_task);
		...
		updataEntity(current_step.getSc_TableName(),
				StringUtil.isMapEmpty(maps)
						? NameQueryUtil.setParams_s(new String[] { "step_id", "step", "step_name", "conclusion", "id" },
								new Object[] { current_step.getScId(), String.valueOf(stepCondition.getTo_next()),
										"申请人", stepCondition.getConclusion(), p_parms_bean.getEntity_id() })
						: maps);
		pushController.pushMessage(p_parms_bean.getSqr_id(), 1);
		return ResultBean.getSucess("sucess");
	}
	 ...
	 * @return
	 */
	public String to_99(TaskExt current_task, ProcessBean p_parms_bean, ProcessStep current_step,
			StepCondition stepCondition, Map<String, Object> maps) {
		// 保存意见
		saveComment(NameQueryUtil.setParams_s("step_id", current_step.getId(), "key_value", current_task.getKey_value(),
				"shyj", p_parms_bean.getShyj(), "shjl", stepCondition.getConclusion(), "sqr_id",
				p_parms_bean.getSqr_id(), "organ_id", p_parms_bean.getOrgan_id()));
		...
		updataEntity(current_step.getSc_TableName(),
				StringUtil.isMapEmpty(maps)
						? NameQueryUtil.setParams_s(new String[] { "step_id", "step", "step_name", "conclusion", "id" },
								new Object[] { current_step.getScId(), stepCondition.getTo_next(), "流程结束",
										stepCondition.getConclusion(), p_parms_bean.getEntity_id() })
						: maps);
		pushController.pushMessage(p_parms_bean.getSqr_id(), 1);
		// 抄送申请人 如果=88 抄送设置的人员
		List<String> us = new ArrayList<String>();
		us.add(current_task.getSqr_id());
		if (stepCondition.getTo_next() == 88) {
			ProcessStep sup_step = (ProcessStep) baseService.findById(ProcessStep.class, current_step.getScId());
			List<String> uxs = getCsrs(sup_step, null);
			if (!StringUtil.isListEmpty(uxs)) {
				us.addAll(uxs);
			}
		}
		saveRemind(current_task, us, "抄送");
		return ResultBean.getSucess("sucess");
	}
	 ...
	 * @return
	 */
	public String to_gq(TaskExt current_task, ProcessBean p_parms_bean, ProcessStep current_step,
			StepCondition stepCondition, Map<String, Object> maps) {
		// 保存意见
		saveComment(NameQueryUtil.setParams_s("step_id", current_step.getId(), "key_value", current_task.getKey_value(),
				"shyj", p_parms_bean.getShyj(), "shjl", stepCondition.getConclusion(), "sqr_id",
				p_parms_bean.getSqr_id(), "organ_id", p_parms_bean.getOrgan_id()));
		// 处理原任务
		current_task.setTaskType(TaskType.挂起);
		current_task.setTime_out(null);
		baseService.save(current_task);
		// 更新实体
		updataEntity(current_step.getSc_TableName(),
				StringUtil.isMapEmpty(maps)
						? NameQueryUtil.setParams_s(new String[] { "step_id", "step", "step_name", "conclusion", "id" },
								new Object[] { current_step.getScId(), stepCondition.getTo_next(), "流程挂起",
										stepCondition.getConclusion(), p_parms_bean.getEntity_id() })
						: maps);
		return ResultBean.getSucess("sucess");
	}
	 ...
	 * @return
	 */
	public String to_next(TaskExt current_task, ProcessBean p_parms_bean, ProcessStep current_step,
			StepCondition stepCondition, List<String> userIds, Map<String, Object> maps) {
		if (StringUtil.isListEmpty(userIds)) {
			return ResultBean.getResultBean("400", "未找到下一步办理人", "未找到下一步办理人");
		}
		...
		saveTask(userIds,
				NameQueryUtil.setParams_s("step_id", next_step.getId(), "key_value", current_task.getKey_value(),
						"sqr_id", current_task.getSqr_id(), "previous_id", p_parms_bean.getSqr_id()));
		// 创建意见
		saveComment(NameQueryUtil.setParams_s("step_id", current_step.getId(), "key_value", current_task.getKey_value(),
				"shyj", p_parms_bean.getShyj(), "shjl", stepCondition.getConclusion(), "sqr_id",
		...
		updataEntity(current_step.getSc_TableName(),
				StringUtil.isMapEmpty(maps)
						? NameQueryUtil.setParams_s(new String[] { "step_id", "step", "step_name", "conclusion", "id" },
								new Object[] { current_step.getScId(), String.valueOf(next_step.getStep()),
										next_step.getName(), stepCondition.getConclusion(),
										p_parms_bean.getEntity_id() })
						: maps);
		pushController.pushMessage(p_parms_bean.getSqr_id(), 1);
		return ResultBean.getSucess("sucess");
	}
	 ...
	 * @return
	 */
	public String to_hq(TaskExt current_task, ProcessBean p_parms_bean, ProcessStep current_step, ProcessStep next_step,
			StepCondition stepCondition, List<String> userIds, Map<String, Object> maps) {
		if (StringUtil.isListEmpty(userIds)) {
			return ResultBean.getResultBean("400", "未找到下一步办理人", "未找到下一步办理人");
		}
		...
		for (String userId : userIds) {
			saveTask(Arrays.asList(new String[] { userId }),
					NameQueryUtil.setParams_s("step_id", next_step.getId(), "key_value", current_task.getKey_value(),
							"sqr_id", current_task.getSqr_id(), "previous_id", p_parms_bean.getSqr_id()));
		}
	...
	public String hq(TaskExt current_task, ProcessBean p_parms_bean, ProcessStep current_step, ProcessStep next_step,
			StepCondition stepCondition, List<String> userIds, Map<String, Object> maps) {
		// 创建意见
		saveComment(NameQueryUtil.setParams_s("step_id", current_step.getId(), "key_value", current_task.getKey_value(),
				"shyj", p_parms_bean.getShyj(), "shjl", stepCondition.getConclusion(), "sqr_id",
		...
		current_task.setTaskType(TaskType.已办);
		baseService.save(current_task);
		List<TaskExt> tasks = (List<TaskExt>) baseService.getList("TaskExt", "", true,
				NameQueryUtil.setParams("key_value", current_task.getKey_value(), "step_id", current_task.getStep_id(),
						"taskType", Arrays.asList(new TaskType[] { TaskType.待办, TaskType.待领取, TaskType.已领取 })));
			...
			updataEntity(current_step.getSc_TableName(),
					StringUtil.isMapEmpty(maps)
							? NameQueryUtil.setParams_s(
									new String[] { "step_id", "step", "step_name", "conclusion", "id" },
									new Object[] { current_step.getScId(), String.valueOf(next_step.getStep()),
											next_step.getName(), "会签完成", p_parms_bean.getEntity_id() })
							: maps);
		}
		pushController.pushMessage(p_parms_bean.getSqr_id(), 1);
		return ResultBean.getSucess("sucess");
	...
	protected ProcessViewBean initProcessViewBean(ProcessBean bean, List<ProcessStep> ps) {
		...
		if (!StringUtil.isEmpty(bean.getEntity_id())) {
			i = (int) baseService.getSimple(p.getTable_name(), "", true, "o.step",
					NameQueryUtil.setParams("id", bean.getEntity_id()));
		}
	...
	protected List<CommentBean> initSteps(ProcessBean bean, List<ProcessStep> ps, ProcessViewBean vbean) {
		Map<String, ProcessStep> map = new HashMap<String, ProcessStep>();
		...
		cb.setStep(0);
		scs.add(cb);
		ProcessStep ks = new ProcessStep();
		if (bean.getStep_id().equals("0005")) {
			ks.setName("验收申请");
			ks.setStep(0);
		} else {
			ks.setName("申请");
		...
		map.put(ps.get(0).getScId() + "0000", ks);
		for (ProcessStep processStep : ps) {
			String key = processStep.getId();
			// String value = processStep.getName();
			map.put(key, processStep);

			CommentBean cbx = new CommentBean();
			cbx.setName(processStep.getName());
			cbx.setStep(processStep.getStep());
			scs.add(cbx);
		}
		CommentBean cby = new CommentBean();
		cby.setName("结束");
		cby.setStep(scs.size());
		scs.add(cby);
		vbean.setPrcocess_steps(scs);

		Map<String, String> maps = new HashMap<String, String>();
		// 查询意见
		List<Comment> comments = (List<Comment>) baseService.getList("Comment", "task", "findComment",
				NameQueryUtil.setParams("key_value", bean.getEntity_id(), "task_id", ps.get(0).getScId()));
		if (!StringUtil.isListEmpty(comments)) {
			...
			for (Comment comment : comments) {
				...
				b.setName(map.get(key).getName());
				b.setStep(map.get(key).getStep());
		...
		// 查询当前流程
		List<TaskExt> tasks = (List<TaskExt>) baseService.getList("TaskExt", "task", "findCurrentTask",
				NameQueryUtil.setParams("key_value", bean.getEntity_id(), "task_id", ps.get(0).getScId()));
		if (!StringUtil.isListEmpty(tasks)) {
			for (TaskExt task : tasks) {
				CommentBean b = new CommentBean();
				if (!map.containsKey(task.getStep_id())) {
					continue;
				}
				b.setName(map.get(task.getStep_id()).getName());
				b.setStep(map.get(task.getStep_id()).getStep());
			...
			for (CommentBean b : scs) {
				if (b.getStep() > tasks.get(0).getStep()) {
					CommentBean x = new CommentBean();
					x.setName(b.getName());
					x.setStep(b.getStep());
					cs.add(x);
				}
	...
	protected List<ProcessStep> getProcessStep(ProcessBean bean) {
		...
		ps = (List<ProcessStep>) baseService.getList("ProcessStep", "o.step asc", true,
				NameQueryUtil.setParams("scId", scId));
		return ps;
	}
	...
	protected TaskExt saveTask(List<String> userIds, Map<String, String> map) {
		...
		pushController.pushTask(userIds, task.getTask_name(), "您有一条待办任务，请尽快处理！");
		return task;
	}
	...
	protected void saveRemind(TaskExt task, List<String> userIds, String r_type) {
		if (!StringUtil.isListEmpty(userIds)) {
			for (String userId : userIds) {
				Remind r = new Remind();
				...
				r.setR_state("0");
				baseService.save(r, false);
			}
			pushController.pushRemind(userIds, task.getTask_name(), "您有一条抄送提醒，请查看！");
		}
	}

	@SuppressWarnings("rawtypes")
	protected void updataEntity(String table_name, Map<String, Object> map) {
		String update_sql = "update " + table_name + " t " + "set ";
		// 根据map拼接参数
	...
	/**
	 * 根据 ProcessStep 获取处理人 1.当直接选定用户时，取用户。
	 ...
	 * 指定部门，则根据指定部门查找角色；不指定部门，则查询拥有该角色的所有人。 4.其他，做单独处理。
	 *
	 * @param p
	 * @return
	 */
	...
	protected List<String> getSprs(ProcessStep p, String org_id) {
		...
		if ("1".equals(p.getShr_type())) {
			...
			// 根据部门和角色 查询审核人
			userIds = (List<String>) baseService.getList("User", "system", "User_by_OrganAndRole_ids",
					NameQueryUtil.setParams("orgs", organs, "roles", p.getRole_s()));
		}
		return userIds;
	}

	@SuppressWarnings("unchecked")
	protected List<String> getCsrs(ProcessStep p, String org_id) {
		List<String> userIds = new ArrayList<String>();
		if ("1".equals(p.getCsr_type())) {
			userIds = p.getCs_user_s();
		} else if ("3".equals(p.getCsr_type())) {
			if (StringUtil.isEmpty(p.getCs_role_type())) {
				userIds = getLd_Users(p.getCs_ld_role_s());
			} else {
				userIds = getLd_Bm_Users(p.getCs_ld_role_s(), org_id);
			}
		} else if ("2".equals(p.getCsr_type())) {
			List<String> organs = new ArrayList<String>();
			// 审核人类型为角色
			if ("1".equals(p.getCs_role_type())) {
				// 上级部门
				Organ o = (Organ) baseService.findById(Organ.class, org_id);
				if (StringUtil.isEmpty(o.getScId())) {
					// 当上级部门为空时，取本级部门
			...
			} else if ("2".equals(p.getCs_role_type())) {
				// 本级部门
				organs.add(org_id);
			} else if ("3".equals(p.getCs_role_type())) {
				// 指定部门
				organs.addAll(p.getCs_organ_s());
			}
			// 根据部门和角色 查询审核人
			userIds = (List<String>) baseService.getList("User", "system", "User_by_OrganAndRole_ids",
					NameQueryUtil.setParams("orgs", organs, "roles", p.getCs_role_s()));
		}
		return userIds;
	}
	...
	protected List<String> getLd_Users(List<String> roles) {
		List<String> userIds = new ArrayList<String>();
		userIds = (List<String>) baseService.getList("Sys_User_Role", null, false, "user_id",
				NameQueryUtil.setParams("role", roles));
		if (!StringUtil.isListEmpty(userIds)) {
			userIds = (List<String>) baseService.getList("SYS_USERS", null, true, "id",
					NameQueryUtil.setParams("id", userIds));
		}
		return userIds;
	}
	...
	protected List<String> getLd_Bm_Users(List<String> roles, String organ_id) {
		...
		List<String> rolexs = (List<String>) baseService.getList("Sys_Role_Organ", null, true, "role",
				NameQueryUtil.setParams("org_id", organ_id, "role", roles));
