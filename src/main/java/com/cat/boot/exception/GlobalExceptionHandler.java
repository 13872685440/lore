	...
	private void handleErrorInfo(HttpServletRequest request, String message, Exception exception) {
		try {
			...
			baseService.noAnnotationUpdate(baseService.query(sql, true).setParameter("id", UUID.randomUUID().toString())
					.setParameter("userId", baseService.getUserInfo(request).getId())
					.setParameter("username", baseService.getUserInfo(request).getName())
					.setParameter("controllermethod", request.getRequestURI()).setParameter("message", message)
